...
function $loader_show(iColor) {
    ...
    var as_myHtml = document.createElement('div');
    as_myHtml.classList.add("myHtml");
    as_myHtml.innerHTML = `<div><i class="material-icons">security</i><span></div>`;
    as_myHtml.style.overflow = "hidden";

    var as_scanning = document.createElement('div');
    as_scanning.classList.add("scanning");
    as_scanning.innerHTML = `<div>Scanning Document for virus</div>`;
    as_scanning.style.overflow = "hidden";


    var as_iconDiv = document.createElement('div');
    as_iconDiv.classList.add("iconDiv");
    as_iconDiv.innerHTML = ` <svg fill="#B4B4B4" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
 ...
 `;
    ...
    as_loaderBox.prepend(as_myHtml);        
    as_loaderBox.appendChild(as_iconDiv);
    as_loaderBox.appendChild(as_scanning);        
    ...
    style.innerHTML = ` .as-background{ position: fixed; width: 100%; height: 100%; top: 0; left: 0; background-color: rgba(0, 0, 0, 0.6); font-family: 'Heebo', sans-serif; z-index: 100000; } 
    .loaderBox {
        position: absolute;
        top: 50%;
        left: 50%;
    ...
    .loaderBox .iconDiv{ position: relative; width: 60px; height: 60px; animation: rotate 1.5s linear infinite } @keyframes rotate{ 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } } .loaderBox .iconDiv svg{ fill: ${iColor}; } `;
    document.head.appendChild(style);
}
